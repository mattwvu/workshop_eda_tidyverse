...
## Computing and Re-coding Variables ----------------------------------------
    ...
    Using the combination of subsetting($) and  =  will allow you to duplicate an existing variable.
    
    
    df$new_varaiable_name = df$listed_varaiable
  
```{r}

    bigfoot$temperature_high_celcius = bigfoot$temperature_high
    view(bigfoot)

```
    
    You can also run computations on the new variable using this method

```{r}

     bigfoot$temperature_high_celcius = (bigfoot$temperature_high -32) * 5/9
      summary(bigfoot$temperature_high)
      summary(bigfoot$temperature_high_celcius)
...
##### HELPFUL HINT ##########
    ...
    If your created variable name is the same as an existing variable in the data frame, it will override the observations in the current variable
...
```
    ...
    Re-coding a numerical variable using the same method we did to create a duplicate variable except this time we are going to use [] to set our parameters
...
```{r}
  ...
  summary(coalProd$Average.Employees)
    ...
    coalProd$Employees_cat_num[coalProd$Average.Employees < 15] = 1
    coalProd$Employees_cat_num[coalProd$Average.Employees >= 15 & coalProd$Average.Employees <= 500] = 2
    coalProd$Employees_cat_num[coalProd$Average.Employees > 500] = 3
...
```
    ...
    Let's then use the mutate function tp rename the variables into categorical names

```{r}

coalProd <- mutate(coalProd, Employees_cat = recode_factor(Employees_cat_num, "1" = "Under 15", "2" = " 15 to 500", "3" = "Over 500", .ordered = T ))
